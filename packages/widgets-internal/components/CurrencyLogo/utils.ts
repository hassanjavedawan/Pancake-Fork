...
export const getTokenListBaseURL = (chainId: number) => `https://tokens.sgcdex.finance/images/${chainName[chainId]}`;

export const getTokenListTokenUrl = (token: Pick<Token, "chainId" | "address">) =>
  Object.keys(chainName).includes(String(token.chainId))
    ? `https://tokens.sgcdex.finance/images/${token.chainId === ChainId.BSC ? "" : `${chainName[token.chainId]}/`}${
        token.address
      }.png`
    : null;
...
export const getCommonCurrencyUrlBySymbol = memoize(
  (symbol?: string): string | undefined =>
    symbol && commonCurrencySymbols.includes(symbol)
      ? `https://tokens.sgcdex.finance/images/symbol/${symbol.toLocaleLowerCase()}.png`
      : undefined,
